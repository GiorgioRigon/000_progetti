...
- nelle bozze outreach `ethics` non dare mai per scontato di avere capito il bisogno del cliente e non implicare che il sistema esistente sia inefficace o insufficiente. Il framing corretto e prudente e: l'organizzazione potrebbe avere bisogno di alleggerire o ordinare meglio alcuni passaggi operativi, ma potrebbe anche avere gia strumenti efficaci; `E-docs` va quindi proposto come possibile supporto operativo, non come correzione necessaria di una carenza presunta
- per realta locali gia organizzate ma non troppo corporate, usare un tono piu morbido e semplice, salvato come tono `Insieme`: meno assertivo, meno tecnico, piu rispettoso dell'esperienza gia maturata dal cliente, con frasi come `immaginiamo che siate gia a posto` e con accenni leggeri a verifiche periodiche o rinnovo senza presentarli come problema esplicito. Questo tono e utile anche per altre organizzazioni locali simili
